...
def test_cxn() -> None:
    ...
    cli_cert  = ca.leaf_cert( encode.person_name("Tim Tester",
                                                  "tim@test.org"),
                               encode.SAN(emails=["tim@test.org"]) )
    with pytest.raises(AssertionError):
        srv_cert  = ca.leaf_cert(name, encode.SAN(hosts=["localhost"]))
    srv_cert  = ca.leaf_cert(name2, encode.SAN(hosts=["localhost"]))
